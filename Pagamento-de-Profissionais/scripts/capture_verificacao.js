...
(async () => {
  ...
  await modal.locator('input[list="apagarFavorecidoFiltroList"]').click();
  const opcoes = await page.locator('#apagarFavorecidoFiltroList option').all();
  let opFav = null;
  for (const o of opcoes) {
    const v = await o.getAttribute('value');
    if (v && v.toLowerCase().includes('profissional 01')) { opFav = v; break; }
  }
  if (opFav) {
    await modal.locator('input[list="apagarFavorecidoFiltroList"]').fill(opFav);
    await modal.locator('input[list="apagarFavorecidoFiltroList"]').blur();
  }
  await page.click('.modal.show button:has-text("Aplicar")');
  await page.waitForTimeout(1000);
  await page.click('.modal.show button:has-text("Fechar")').catch(() => {});
  await page.waitForTimeout(400);
  await shot(page, '10-conta-a-pagar-gerada-pelo-pagamento-profissional');
