...
![Tela inicial (Home) do administrador: atalhos para Agenda, Doc Faturamento, Prontuário, Caixa, Protocolo e Check-In. O menu lateral dá acesso a todos os módulos.](a01-home.png)

_Tela inicial (Home) do administrador: atalhos para Agenda, Doc Faturamento, Prontuário, Caixa, Protocolo e Check-In. O menu lateral dá acesso a todos os módulos._
...
![Fluxo geral: Cadastros → Agenda → Atendimento e Prontuário → Financeiro.](h01-fluxo.png)

_Fluxo geral: Cadastros → Agenda → Atendimento e Prontuário → Financeiro._
...
![Contas a Receber: cartões de resumo (Em Aberto, Vencido, Vence Hoje, A Vencer, Recebido no Mês) e a lista de cobranças. Contas a Pagar segue o mesmo modelo.](f05-contas-receber.png)

_Contas a Receber: cartões de resumo (Em Aberto, Vencido, Vence Hoje, A Vencer, Recebido no Mês) e a lista de cobranças. Contas a Pagar segue o mesmo modelo._

![Prontuário: o profissional (ou o administrador) filtra e abre os prontuários por tipo, profissional, paciente e período.](f03-prontuario.png)

_Prontuário: o profissional (ou o administrador) filtra e abre os prontuários por tipo, profissional, paciente e período._
...
![Cadastros → Clínica: cartões com totais de Pacientes, Profissionais e Acessos (ativos/inativos).](c01b-clinica-topo.png)

_Cadastros → Clínica: cartões com totais de Pacientes, Profissionais e Acessos (ativos/inativos)._
...
![Cadastros → Setores: nome e situação (ativo/inativo) de cada setor.](c02-setores.png)

_Cadastros → Setores: nome e situação (ativo/inativo) de cada setor._
...
![Tabelas Aux. → Especialidade: descrição, abreviação, tipo de cobrança, compartilhamento de relatório e situação.](c03-especialidades.png)

_Tabelas Aux. → Especialidade: descrição, abreviação, tipo de cobrança, compartilhamento de relatório e situação._
...
![Cadastros → Operadora → Listar: convênios cadastrados.](c08-operadoras.png)
...
![Novo Acesso: o primeiro passo é pesquisar pelo CPF.](c09b-acesso-novo.png)

_Novo Acesso: o primeiro passo é pesquisar pelo CPF._
...
![Menu de ações do usuário: Editar, Desativar, Permissões, bloquear dias e Resetar para senha padrão.](c10-acesso-menu.png)

_Menu de ações do usuário: Editar, Desativar, Permissões, bloquear dias e Resetar para senha padrão._
...
![Editar: perfil, perfil secundário, setor e a chave "Controla Caixa".](c11-acesso-editar.png)

_Editar: perfil, perfil secundário, setor e a chave "Controla Caixa"._

![Permissões do usuário: adicionar permissões extras.](c12-acesso-permissoes.png)

_Permissões do usuário: adicionar permissões extras._

![Bloqueio de dias: marque os dias da semana em que o usuário pode acessar.](c13-acesso-bloqueio-dias.png)

_Bloqueio de dias: marque os dias da semana em que o usuário pode acessar._
...
![Nova Agenda: mês, ano e a opção de copiar uma agenda anterior.](b01-nova-agenda.png)

_Nova Agenda: mês, ano e a opção de copiar uma agenda anterior._
...
![Configuração de Horários: lista com situação, botão Ativar/Desativar, campo para novo horário e "Gerar Horários".](b02-horarios.png)

_Configuração de Horários: lista com situação, botão Ativar/Desativar, campo para novo horário e "Gerar Horários"._
...
![Gerar Horários: exemplo de 08:00 às 18:00 de 30 em 30 minutos.](b03-gerar-horarios.png)

_Gerar Horários: exemplo de 08:00 às 18:00 de 30 em 30 minutos._
...
![Primeiro acesso: apresentação do novo layout da tela de horários.](b04-prof-horarios-apresentacao.png)

_Primeiro acesso: apresentação do novo layout da tela de horários._
...
![Horários: Seg e Qua marcados, período da Manhã selecionado e, à direita, a grade da semana com os horários já cadastrados.](b05-prof-horarios-selecionado.png)

_Horários: Seg e Qua marcados, período da Manhã selecionado e, à direita, a grade da semana com os horários já cadastrados._
...
![Menu Mais ações: transferir, clonar ou remover todos os horários do profissional.](b06-prof-horarios-mais-acoes.png)

_Menu Mais ações: transferir, clonar ou remover todos os horários do profissional._
...
![Grade de agendamento: horários dos profissionais, pacientes vinculados, plano, sessões e status de cada sessão.](d01-agendamento.png)

_Grade de agendamento: horários dos profissionais, pacientes vinculados, plano, sessões e status de cada sessão._
...
![Vincular Paciente: busca do paciente cadastrado (nome, data de nascimento, mãe, pai e município).](d04-vincular-busca.png)

_Vincular Paciente: busca do paciente cadastrado (nome, data de nascimento, mãe, pai e município)._

![Formulário do agendamento preenchido: dados do horário, do paciente e as informações do atendimento.](d06-agendar-form-preenchido.png)

_Formulário do agendamento preenchido: dados do horário, do paciente e as informações do atendimento._
...
![Menu de ações da linha: Editar, Sessão, Adicionar mais, Desmarcar, Entrada, Saída, Trocar Profissional e Horários Pac.](d02-agendamento-menu-linha.png)

_Menu de ações da linha: Editar, Sessão, Adicionar mais, Desmarcar, Entrada, Saída, Trocar Profissional e Horários Pac._
...
![Marcar Sessão: identifica profissional, paciente, dia e hora e pede a sessão a marcar.](d07-sessao.png)

_Marcar Sessão: identifica profissional, paciente, dia e hora e pede a sessão a marcar._
...
_Filtros da grade de agendamento._
...
![Janela Relatórios: data da agenda e tipo de relatório.](d09b-relatorios-presenca.png)

_Janela Relatórios: data da agenda e tipo de relatório._
...
![PDF do Atendimento Diário.](e-atendimento-diario.png)

_PDF do Atendimento Diário._
...
![PDF da Lista de presença do dia.](e-presenca-ausencia.png)

_PDF da Lista de presença do dia._
...
![Marcação do mês/dia: usuário responsável, status e totais.](e03-marcacao-sessao-dia.png)

_Marcação do mês/dia: usuário responsável, status e totais._
...
![PDF Horários do paciente (trecho superior).](e-horarios-paciente.png)

_PDF Horários do paciente (trecho superior)._

## Relatório da Agenda e relatórios no menu Relatórios

![Agenda → Relatório: cartões-resumo do mês, com filtro por profissional.](e05-relatorio-agenda.png)

_Agenda → Relatório: cartões-resumo do mês, com filtro por profissional._

![Menu Relatórios: o bloco "Agenda" reúne os relatórios de acompanhamento.](f01-relatorios.png)

_Menu Relatórios: o bloco "Agenda" reúne os relatórios de acompanhamento._

![Exemplo: Relatório - Presença Diária, com filtros de usuário, profissional, paciente, data, particular e método.](g01-presenca-diaria.png)

_Exemplo: Relatório - Presença Diária, com filtros de usuário, profissional, paciente, data, particular e método._
...
# Anexo técnico (somente versão completa)

| Assunto | Detalhe técnico |
|---|---|
| Cópia da agenda | Ao criar com "Copiar agenda anterior", a API chama a stored procedure AgendaAbertura, que cria a linha em Agenda (com IdAgendaCopy apontando para a origem), copia AgendaProfissionalDia (horários) e AgendaProfissionalDiaPac (pacientes vinculados) e, nesta última, zera IDSTATUS_S1..S5 (-1) e DATAS1..S5 (nulas). |
| Modo de cópia | O campo Clinica.CopiaAgenda define o que é copiado: T = tudo, S = só horários semanais (Data = 1900-01-01), D = só horários por data específica. |
| Unicidade | A API impede duas agendas no mesmo mês/ano da clínica (mensagem "Já existe uma agenda criada para..."). |
| Horários da clínica | Tabela de horários ativos consultada na montagem da grade de Horários do profissional (tela Tabelas Aux. → Horários, rota /aux/horarios). |
