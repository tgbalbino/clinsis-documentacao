...
(async () => {
  ...
  // Lista de Agendas (meses)
  await page.goto(`${APP}/agenda`);
  await page.waitForLoadState('networkidle');
  await esconderMenuTestes();
  await page.waitForTimeout(700);
  await shot(page, '20-lista-agendas-meses');

  // Clicar em "Ver Agenda" da linha 2026/SETEMBRO
  const linhaSet = page.locator('tr', { hasText: '2026 / SETEMBRO' }).first();
  await linhaSet.locator('a, button', { hasText: 'Ver Agenda' }).click();
  let idAgenda = null;
  await page.waitForTimeout(1200);
  idAgenda = new URL(page.url()).searchParams.get('idAgenda');
  console.log('idAgenda detectado:', idAgenda, page.url());
  await esconderMenuTestes();
  await shot(page, '21-agendamento-mes');

  if (idAgenda) {
    require('fs').writeFileSync(path.join(__dirname, 'idagenda.txt'), idAgenda);
  }
