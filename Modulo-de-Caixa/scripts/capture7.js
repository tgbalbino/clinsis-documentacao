...
(async () => {
  ...
  await novaPagina.addStyleTag({ content: '#li_testes { display: none !important; }' });
  await novaPagina.waitForTimeout(1000);
  await novaPagina.screenshot({ path: path.join(outDir, '22-extrato-caixa-fechado.png'), fullPage: true });
  await novaPagina.close();

  // ---- Listar (historico geral) ----
  await page.goto(`${APP}/caixas`);
  await page.waitForLoadState('networkidle');
  await esconderMenuTestes();
  await page.waitForTimeout(800);
  await shot(page, '23-listar-historico-caixas');
