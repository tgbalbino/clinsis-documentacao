...
(async () => {
  ...
  // Metodo Pagto (1o select) -> Dinheiro (evita restricao de conta financeira x forma)
  const metodoSelect = modalBaixa.locator('select').first();
  const opcoesMetodo = await metodoSelect.locator('option').all();
  let valorDinheiro = null;
  for (const o of opcoesMetodo) {
    const t = (await o.textContent() || '').toLowerCase();
    if (t.includes('dinheiro')) { valorDinheiro = await o.getAttribute('value'); break; }
  }
  if (valorDinheiro) await metodoSelect.selectOption(valorDinheiro);
  await page.waitForTimeout(300);

  // Valor = Valor Restante (25,00)
  const campoValor = modalBaixa.locator('input[mask="separator.2"]').first();
  await campoValor.fill('25,00');
  await page.waitForTimeout(200);

  // Conta financeira (2o select) - primeira opcao disponivel
  const contaSelect = modalBaixa.locator('select').nth(1);
  const opcoesConta = await contaSelect.locator('option').all();
  if (opcoesConta.length > 1) {
    const v = await opcoesConta[1].getAttribute('value');
    await contaSelect.selectOption(v);
  }
  await page.waitForTimeout(300);
