...
(async () => {
  ...
  await page.goto(`${APP}/acessos`);
  await page.waitForLoadState('networkidle');
  await esconderMenuTestes();
  await page.waitForTimeout(800);
  await page.screenshot({ path: 'debug-acessos.png', fullPage: true });

  const linhaAdmin = page.locator('tr').filter({ hasText: 'ADMIN' }).first();
  await linhaAdmin.locator('button').first().click();
  await page.waitForTimeout(700);
  await page.screenshot({ path: 'debug-acessos-editar.png', fullPage: true });

  const chk = page.locator('#sw-ControlaCaixa');
  const marcado = await chk.isChecked();
  console.log('ControlaCaixa atual:', marcado);
  if (!marcado) {
    await chk.check();
  ...
  await page.screenshot({ path: 'debug-acessos-salvo.png', fullPage: true });
