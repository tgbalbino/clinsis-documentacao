...
(async () => {
  ...
  await shot(page, '11-extrato-completo-antes-fechar');

  // ---- Solicitar cancelamento do lancamento de suprimento (pra depois aprovar em Solicitacoes) ----
  const linhaSuprimento = page.locator('tr').filter({ hasText: 'Suprimento' });
  await linhaSuprimento.locator('button:has-text("Cancelar")').click();
  await page.waitForTimeout(500);
  await shot(page, '12-confirmar-solicitar-cancelamento');
  await page.locator('button:visible', { hasText: 'Sim' }).click();
  await page.waitForTimeout(1000);
  await shot(page, '13-apos-solicitar-cancelamento');
  ...
  // ---- Meus Caixas (historico do proprio usuario) ----
  await page.waitForTimeout(500);
