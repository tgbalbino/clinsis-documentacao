...
(async () => {
  ...
  await shot(page, '01-modal-abrir-caixa');
  await page.fill('.meuModalContainer input[mask="separator.2"]', '100,00');
  await page.waitForTimeout(300);
  await shot(page, '02-abrir-caixa-preenchido');
  await page.click('button:has-text("Confirmar")');
  await page.waitForTimeout(1200);
  await shot(page, '03-caixa-aberto-extrato-vazio');

  // ---- Novo lancamento: Suprimento ----
  await page.click('button:has-text("Novo Lançamento")');
  await page.waitForTimeout(600);
  await page.selectOption('.meuModalContainer select', 'E');
  await page.waitForTimeout(400);
  await shot(page, '04-modal-lancamento-tipo-suprimento');
  const formaSelect = page.locator('.meuModalContainer select').nth(1);
  await formaSelect.selectOption({ index: 1 });
  await page.fill('.meuModalContainer input[type="text"]:below(label:has-text("Observação"))', 'Troco inicial adicional').catch(async () => {
  ...
  await page.click('.meuModalContainer button:has-text("+")').catch(async () => {
