...
(async () => {
  ...
  await page.goto(`${APP}/aux/especialidade`);
  await page.waitForLoadState('networkidle');
  await esconderMenuTestes();
  await page.waitForTimeout(700);
  await shot(page, '00-lista-especialidades');

  // Editar a primeira especialidade pra ver os campos preenchidos
  await page.locator('table tbody tr').first().locator('button.btn-warning, button[title*="Editar" i]').first().click();
  await page.waitForSelector('.modal.show');
  await page.waitForTimeout(600);
  await shot(page, '01-cadastro-tipo-cobranca-rel-compartilhado');

  await page.click('.modal.show button:has-text("Fechar"), .modal.show button:has-text("Cancelar")').catch(() => {});
  await page.waitForTimeout(400);

  // Prontuario: combo Emitente
  await page.goto(`${APP}/prontuario/profissional`);
  await page.waitForLoadState('networkidle').catch(() => {});
  await esconderMenuTestes();
  await page.waitForTimeout(1000);
  await shot(page, '03-prontuario-lista');
