const path = require('path');
const { chromium } = require('playwright');

const API = process.env.E2E_API_URL || 'http://localhost:49020/api';
const APP = process.env.E2E_APP_URL || 'http://localhost:4222';
const LOGIN = process.env.E2E_LOGIN || '99999999999';
const SENHA = process.env.E2E_SENHA || 'Cli99999';
const outDir = path.join(__dirname, 'screenshots');
const shot = (page, name, fullPage = true) => page.screenshot({ path: path.join(outDir, `${name}.png`), fullPage });

async function loginApi() {
  const r1 = await fetch(`${API}/usuario/login`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ Login: LOGIN, Senha: SENHA })
  });
  const j1 = await r1.json();
  const r2 = await fetch(`${API}/clinicausuario/SelecionarClinica`, {
    method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${j1.data.token}` },
    body: JSON.stringify({ IdUsuario: j1.data.id, IdClinica: 1, IdPerfilEscolha: 0 })
  });
  const j2 = await r2.json();
  return { token: j2.data.token, nome: j2.data.nome, id: j2.data.id, papel: j2.data.papel, clinicaConfig: j2.data.clinicaConfig || [] };
}

(async () => {
  const auth = await loginApi();
  const browser = await chromium.launch();
  const page = await browser.newPage({ viewport: { width: 1600, height: 1100 } });
  page.setDefaultTimeout(30000);
  const esconderMenuTestes = () => page.addStyleTag({ content: '#li_testes { display: none !important; }' });

  await page.goto(APP);
  await page.evaluate((a) => {
    localStorage.setItem('_clTk', a.token);
    localStorage.setItem('_clNm', JSON.stringify(a.nome));
    localStorage.setItem('_clId', JSON.stringify(String(a.id)));
    localStorage.setItem('_clType', JSON.stringify(a.papel));
    localStorage.setItem('_clcfAux', JSON.stringify(a.clinicaConfig));
  }, auth);
  await esconderMenuTestes();

  // ---- Listagem de layouts ----
  await page.goto(`${APP}/aux/contratolaiout`);
  await page.waitForLoadState('networkidle');
  await esconderMenuTestes();
  await page.waitForTimeout(700);
  await shot(page, '00-lista-layouts');

  // ---- Editar o layout ativo (correto, id 2) ----
  const linhaAtiva = page.locator('tr').filter({ has: page.locator('span.badge.bg-success') });
  await linhaAtiva.locator('button:has-text("Editar")').click();
  await page.waitForTimeout(700);
  await shot(page, '01-editor-layout-ativo');

  // rolar ate o final pra mostrar as variaveis disponiveis
  await page.locator('text=Variáveis disponíveis').scrollIntoViewIfNeeded();
  await page.waitForTimeout(300);
  await shot(page, '02-variaveis-disponiveis');

  // ---- Visualizar PDF de exemplo (a partir do layout correto ja aberto) ----
  const pagePromise = page.context().waitForEvent('page', { timeout: 20000 }).catch(() => null);
  await page.click('button:has-text("Visualizar PDF de exemplo")');
  const novaPagina = await pagePromise;
  await page.waitForTimeout(1500);
  if (novaPagina) {
    await novaPagina.waitForLoadState('load').catch(() => {});
    await novaPagina.waitForTimeout(2500);
    await novaPagina.screenshot({ path: path.join(outDir, '05-pdf-exemplo-gerado.png') });
    await novaPagina.close();
  } else {
    await shot(page, '05-pdf-exemplo-gerado');
  }

  // ---- Novo layout + Usar modelo padrao ----
  await page.click('button:has-text("Cancelar")');
  await page.waitForTimeout(500);
  await page.click('button:has-text("Novo layout")');
  await page.waitForTimeout(500);
  await shot(page, '03-novo-layout-vazio');
  await page.click('button:has-text("Usar modelo padrão")');
  await page.waitForTimeout(500);
  await shot(page, '04-modelo-padrao-preenchido');

  await browser.close();
  console.log('OK');
})().catch(e => { console.error('FALHA:', e.message, e.stack); process.exit(1); });
