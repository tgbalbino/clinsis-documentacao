...
(async () => {
  ...
  const linha28 = page.locator('tr').filter({ has: page.locator('td', { hasText: /^28$/ }) });
  await linha28.locator('button[title="Alterar contrato / serviços"]').click();
  await page.waitForSelector('.modal.show', { timeout: 5000 });
  await page.waitForTimeout(600);
  await shot(page, '21-contrato-fechado-antes-nao-renovar');

  await page.click('.modal.show button:has-text("Não vai renovar")');
  await page.waitForTimeout(600);
  await shot(page, '22-confirmar-nao-vai-renovar');
  await page.click('#modalNaoVaiRenovarContrato button:has-text("Sim")');
  await page.waitForTimeout(1200);
  await shot(page, '23-apos-nao-vai-renovar');
