...
(async () => {
  ...
  await shot(page, '25a-contapagar-lista');
  await page.locator('button[title="Pagamento / Acerto"]').first().click();
  await page.waitForSelector('.modal.show');
  await page.waitForTimeout(700);
  const modalBaixa = page.locator('.modal.show').last();
  async function selecionar(select, txt) {
    for (const o of await select.locator('option').all()) {
      if (((await o.textContent()) || '').toLowerCase().includes(txt)) { await select.selectOption(await o.getAttribute('value')); return; }
    }
  }
  await selecionar(modalBaixa.locator('select').first(), 'cartão de crédito');
  await modalBaixa.locator('input[mask="separator.2"]').first().fill('1,00');
  await selecionar(modalBaixa.locator('select').nth(1), 'caixa');
  await page.waitForTimeout(300);
  await shot(page, '25-contapagar-baixa-nao-habilitada', false);
  await modalBaixa.locator('button:has-text("Lançar")').click();
  await page.waitForTimeout(700);
  await shot(page, '25b-apos-lancar', false);
  await page.locator('button:visible', { hasText: 'Sim' }).click();
  await page.waitForTimeout(500);
  await shot(page, '26-contapagar-erro-forma-nao-habilitada', false);
  await browser.close();
  console.log('OK');
