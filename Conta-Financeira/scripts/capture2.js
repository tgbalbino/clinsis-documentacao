...
(async () => {
  ...
  let valorCaixa = null;
  for (const o of opcoesConta) {
    const t = (await o.textContent() || '').toLowerCase();
    if (t.includes('caixa')) { valorCaixa = await o.getAttribute('value'); break; }
  }
  await contaSelect.selectOption(valorCaixa);
  await page.waitForTimeout(300);
  ...
  // ---- Agora com Dinheiro + CAIXA (combinacao valida) ----
  await metodoSelect.selectOption({ label: 'Dinheiro' }).catch(async () => {
    const opcoes2 = await metodoSelect.locator('option').all();
    for (const o of opcoes2) {
      const t = (await o.textContent() || '').toLowerCase();
      if (t.includes('dinheiro')) { await metodoSelect.selectOption(await o.getAttribute('value')); break; }
    }
  });
  await page.waitForTimeout(300);
  await shot(page, '06-baixa-combinacao-valida');
  await modalBaixa.locator('button:has-text("Lançar")').click();
  await page.waitForTimeout(600);
  await page.locator('button:visible', { hasText: 'Sim' }).click().catch(() => {});
  await page.waitForTimeout(1200);
  await shot(page, '07-baixa-com-sucesso');
