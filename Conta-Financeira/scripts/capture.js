...
(async () => {
  ...
  await shot(page, '00-lista-contas-financeiras');

  // ---- Novo cadastro (mostrar campos, sem salvar) ----
  await page.click('button:has-text("Novo")');
  await page.waitForTimeout(500);
  await page.fill('.modal.show input[maxlength="100"]', 'CAIXA CONSULTORIO 2');
  await page.waitForTimeout(300);
  await shot(page, '01-novo-cadastro-preenchido');
  await page.click('.modal.show button:has-text("Fechar")');
  await page.waitForTimeout(500);

  // ---- Formas de pagamento da conta CAIXA: desabilitar Cartao de Credito ----
  const linhaCaixa = page.locator('tr').filter({ hasText: 'CAIXA' }).first();
  await linhaCaixa.locator('button[title="Formas de pagamento desta conta"]').click();
  await page.waitForTimeout(600);
  await shot(page, '02-formas-pagamento-antes');

  const linhaCartao = page.locator('.modal.show tr').filter({ hasText: 'Cartão de Crédito' });
  const temDesabilitar = await linhaCartao.locator('button:has-text("Desabilitar")').count();
  if (temDesabilitar > 0) {
    await linhaCartao.locator('button:has-text("Desabilitar")').click();
    await page.waitForTimeout(400);
    await page.locator('button:visible', { hasText: 'Sim' }).click();
    await page.waitForTimeout(800);
  }
  await shot(page, '03-formas-pagamento-depois');
  await page.click('.modal.show button:has-text("Fechar")');
  await page.waitForTimeout(500);
