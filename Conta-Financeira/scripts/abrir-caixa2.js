...
(async () => {
  ...
  const input = page.locator('.meuModalTotal input[mask="separator.2"]');
  await input.click();
  await input.pressSequentially('10000', { delay: 40 });
  await page.waitForTimeout(400);
  await page.locator('.meuModalTotal button:has-text("Confirmar")').click();
  await page.waitForTimeout(1500);
  await page.screenshot({ path: 'screenshots/abrir-caixa-final.png' });
  await browser.close();
  console.log('OK');
